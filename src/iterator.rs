...
use crate::{db::Engine, index::{IndexIterator}, options::IteratorOptions, errors::Result};
...
impl Iterator<'_> {
    ...
    pub fn next(&mut self) -> Option<(Bytes, Bytes)> {
      ...
      if let Some(item) = index_iter.next() {
        let value = self.engine.get_value_by_position(item.1).expect("failed to get value from data file");
